...
router.post("/voice", express.urlencoded({ extended: false }), async (req, res) => {
  try {
    ...
    twiml.play(await synthesizeSpeech(greeting, owner.voiceId || undefined)); gather(twiml, call._id); twiml.redirect({ method: "POST" }, `${BASE_URL}/api/inbound/no-input?callId=${call._id}`);
...
router.post("/turn", express.urlencoded({ extended: false }), async (req, res) => {
  try {
    ...
    const owner = await User.findById(call.owner); const speech = (req.body.SpeechResult || "").trim(); const twiml = new twilio.twiml.VoiceResponse();
